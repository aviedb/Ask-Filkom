...
export default class Index extends Component {
  ...
  showQuestions(snapshot) {
    const { searchKeyword } = this.props.url.query

    const temp = Object.entries(snapshot)
    ...
    for (var i=0; i<temp.length; i++) {
      ...
      temp[i].tags.map((tag) => {
        if (tag.toUpperCase() === searchKeyword.toUpperCase())
          cek = true
      })
  ...
  render() {
    const { questions, authUser, countSearch } = this.state
      ...
      <MuiThemeProvider muiTheme={muiTheme}>
        <div>
          <Head>
            ...
            <title>Search: "{this.props.url.query.searchKeyword}" - ask-filkom</title>
          </Head>
            ...
            <div className="home__middle">
              ...
              { !this.state.loading &&
                <h3 style={{ padding: "15px" }}>
                  {countSearch} Search Result for: {this.props.url.query.searchKeyword}
                </h3>
              }
              <ul>
                {
